 ...
 UPDATE responsible SET responsible_status = 'C' 
  ...
  INSERT INTO responsible_inactive(ID,ident,NAME,company_id,from_period,to_period,status) 
    SELECT sys_guid(), responsible_ident, responsible_name,company_id,from_period,to_period,responsible_status 
      FROM responsible WHERE responsible_status <> 'N' AND responsible_id <> '344SN';
  
  DELETE FROM responsible WHERE responsible_status <> 'N' AND responsible_id <> '344SN';
...
order by dim_value;
*/
-- M̴ h̴ndteres ,HMS_ORG_LEVEL_REPORT_ID
...
BEGIN
  ...
  */
  ...
  update discrepancy set DK_COMPANY_TREEKEY = 300799 where dk_company_treekey = 3007;
